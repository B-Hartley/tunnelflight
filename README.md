...
This service lists all countries that have wind tunnels in the database. It takes no parameters and displays the results as a persistent notification.
...
This service forces a refresh of all data from the Tunnelflight API. The integration normally updates every 6 hours, but you can use this service to trigger an immediate update.

Example:
```yaml
service: tunnelflight.refresh_data
```
...
```yaml
- alias: Daily Tunnelflight Refresh
  trigger:
    - platform: time
      at: "06:00:00"
  action:
